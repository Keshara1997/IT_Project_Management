<?php
	...
	if($action == 'add'){
		if(isset($_POST['save'])){
				...
				if ($result === TRUE) {
					echo "<script>alert('Set the Time');location='../manageTimetableT.php';</script>";
				} else {
					echo "<script>alert('Error);location='../manageTimetableT.php';</script>";
				}
	...
	if($action == 'delete'){
		...
		if ($result === TRUE) {
			echo "<script>alert('Time Removed');location='../manageTimetableT.php';</script>";
		} else {
			echo "<script>alert('Error);location='../manageTimetableT.php';</script>";
		}
			...
			if($action == 'edit'){
				if(isset($_POST['save'])){
					...
					$result = $mysqli->query("UPDATE tbtimetable SET CHAPTERNAME='$CHAPTER_NAME', CHAPTERLESSON='$CHAPTER_LESSON', DATE='$DATE' WHERE TTID = $TTID AND CID = $CID") or die($mysqli->error);

					if ($result === TRUE) {
						echo "<script>alert('Update Time');location='../manageTimetableT.php';</script>";
					} else {
						echo "<script>alert('Error);location='../manageTimetableT.php';</script>";
					}

...
switch ($action) {
	...
	function doInsert(){
		...
		if ($_POST['user_name'] == "" OR $_POST['user_email'] == "" OR $_POST['user_pass'] == "") {
			...
			echo "<script>alert('New User created successfully!');location='../dashboard.php';</script>";
	...
	function doEdit(){
	    ...
	    if(isset($_POST['save'])){
			...
			$user->update($_POST['user_id']);
			 ...
			 $sql = "SELECT * FROM `tblusers` WHERE `USERID`='".$_POST['user_id']."'";
			 $mydb->setQuery($sql);
			 $mydb->executeQuery();


			  message("[". $_POST['user_name'] ."] has been updated!", "success");
			  echo "<script>alert('User has been updated!');location='../dashboard.php';</script>";
              redirect("../manageUser.php");
		}
	}
	...
	function doDelete(){
		...
		// 	$user->delete($id[$i]);
			...
			echo "<script>alert('User already Deleted!');location='../dashboard.php';</script>";
            redirect("../manageUser.php");
		// }
		// }

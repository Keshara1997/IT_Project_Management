                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card-header">
                                <h2 class="p-1 m-0 text-16 font-weight-semi">Attendents Details</h2>
                            </div>
                            <div class="card">
                                <div class="card-body">
                                <table id="basicDatatable" class="table" style="width:100%">
                                        ...
                                        <tbody>
                                        <?php 
                                            $query = "SELECT * FROM `tbattendent` ORDER BY `tbattendent`.`ATT_ID` ASC";
                                            $mydb->setQuery($query);
                                        ...
                                        ?>
                                            <tr>
                                                <td>
                                                    <?php echo $result->ATTENDENT_NAME ?> 
                                                    <?php 
                                                ...
                                                <td><?php echo $result->ATTENDENT_USER_NAME ?></td>
                                                <td>
                                                    <?php 
                                                        if ($result->ACTIVE == 'DISABLE'){
                                                            echo "<a href='admin/action/attendentActivator.php?status=ACTIVE&id=$result->ATT_ID'><button class='btn btn-opacity-success btn-sm' type='button'>Activate</button></a>";
                                                        }else {
                                                            echo "<a href='admin/action/attendentActivator.php?status=DISABLE&id=$result->ATT_ID'><button class='btn btn-opacity-danger btn-sm' type='button'>Disable</button></a>";
                                                        }
                                                    ?>
                                                    <a href="admin/action/attendentController.php?action=delete&id=<?php echo $result->ATT_ID ?>" onclick="return confirm('Are you sure you want to delete this item?');"><button class="btn text-danger rounded-circle m-0 btn-sm btn-icon"><i class="material-icons">delete</i></button></a>
                                                </td>
                                            </tr>
                                        <?php } ?>   
